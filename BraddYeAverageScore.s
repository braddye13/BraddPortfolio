...
loop_in:
   ...
   # Calculate average (sum / adjusted size)
   div $v0, $a1          # Perform division
   mflo $t1              # Quotient (average)
   mfhi $t2              # Remainder
   # Round up if remainder >= 5
   bge $t2, 5, roundUp
   move $t3, $t1         # Average without rounding
   j printAverage
roundUp:
   addi $t3, $t1, 1      # Rounded up average
printAverage:
   li $v0, 4             # Print message
   la $a0, str5
   syscall
   li $v0, 1             # Print integer (average)
   move $a0, $t3
   syscall
...
move $t0, $a1        # Set counter to array size
la $t1, ($a0)        # Base address of array

printLoop:
beqz $t0, printEnd   # If counter == 0, exit loop
lw $t2, 0($t1)       # Load array element
li $v0, 1
